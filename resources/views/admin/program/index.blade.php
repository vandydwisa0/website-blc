...
@section('content')
    <div class="sm:ml-64">
        <div class="p-4 py-10 mt-14">
            <h1 class="mb-2 text-gray-700 font-bold text-4xl">Data Program</h1>
            ...
            <section class="bg-gray-50 ">
                <div class="flex-auto mx-auto max-w-full">
                    ...
                    <div class="bg-white  relative shadow-md sm:rounded-lg overflow-hidden">
                            ...
                            <table id="myTable" class="min-w-full text-sm text-left text-gray-500 ">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-50  ">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3">Nama Program</th>
                                        <th scope="col" class="px-4 py-3">Jenis Pembayaran</th>
                                        {{-- <th scope="col" class="px-4 py-3">Biaya</th> --}}
                                        <th scope="col" class="px-4 py-3">Jadwal Pertemuan Perminggu</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($program as $item)
                                        <tr class="border-b ">
                                            ...
                                            {{-- <td class="px-4 py-3">{{ $item->data()['price'] }}</td> --}}
                                            <td class="px-4 py-3">{{ $item->data()['meetingsPerWeek'] }}</td>
                                            <td class="flex px-6 py-4 items-center justify-center">
                                                @include('admin.program.edit')
                                                @include('admin.program.delete')
                        ...
                        {{-- PAGINATION --}}
                        {{-- {{ $program->links('vendor.pagination.tailwind') }} --}}
                    </div>
                </div>
            </section>
        </div>
    ...
    <script>
        ...
        function myFunction() {
            ...
            for (i = 0; i < tr.length; i++) {
                // Ganti indeks [0], [1], dan [2] untuk memfilter kolom Name, Payment Type, dan Meetings Per Week
                tdName = tr[i].getElementsByTagName("td")[0];
                tdPaymentType = tr[i].getElementsByTagName("td")[1];
                tdMeetingsPerWeek = tr[i].getElementsByTagName("td")[2];

                if (tdName || tdPaymentType || tdMeetingsPerWeek) {
                    txtValueName = tdName.textContent || tdName.innerText;
                    txtValuePaymentType = tdPaymentType.textContent || tdPaymentType.innerText;
                    txtValueMeetingsPerWeek = tdMeetingsPerWeek.textContent || tdMeetingsPerWeek.innerText;
                    ...
                    if (
                        ...
                        txtValueMeetingsPerWeek.toUpperCase().indexOf(filter) > -1
                    ) {
                        tr[i].style.display = "";
                    } else {
